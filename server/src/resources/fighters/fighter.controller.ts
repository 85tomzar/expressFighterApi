...
import { addFighter, deleteFighterById, getFighterById, getFighters, updateFighterById } from "./fighterdata.handler";
...
export const getFighter = (req: Request, res: Response, next: NextFunction) => {
  ...
  fighter ? res.status(200).json(fighter) : res.status(404).json(`no fighter with id ${req.params.id}`);
};
